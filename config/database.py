...
def init_db(app):
    ...
    with app.app_context():
        try:
            db.create_all()
            db_type = "PostgreSQL" if "postgresql" in app.config[
                'SQLALCHEMY_DATABASE_URI'] else "SQLite"
            logger.info(
                f"✅ Base de datos {db_type} inicializada correctamente")
...
def migrate_to_postgresql():
    ...
    try:
        ...
        postgres_enabled = os.getenv(
            'POSTGRES_ENABLED',
            'False').lower() == 'true'
        if not postgres_enabled:
            logger.error(
                "❌ PostgreSQL no está habilitado en las variables de entorno")
            return False
