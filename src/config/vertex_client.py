...
try:
    ...
    logging.warning(
        "⚠️ Vertex AI no disponible - instalar: pip install google-cloud-aiplatform vertexai"
    )
...
try:
    ...
    logging.warning(
        "⚠️ Gemini API no disponible - instalar: pip install google-generativeai"
    )
...
class VertexAIClient:
            ...
            try:
                if self.config.initialize():
                    ...
                    for model_type, model_info in self.config.models.items():
                        try:
                            self.models[model_type] = GenerativeModel(
                                model_info["name"]
                            )
                            logger.debug(
                                f"✅ Modelo {model_type} ({
                                    model_info['name']}) cargado"
                            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.gemini_client:
            try:
                test_result = await self._generate_with_gemini_api("Test", 10, 0.1)
