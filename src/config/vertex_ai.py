...
class VertexAIConfig:
    ...
    def validate_config(self) -> tuple[bool, str]:
        ...
        if self.credentials_path and not os.path.exists(self.credentials_path):
            return False, f"Archivo de credenciales no encontrado: {self.credentials_path}"
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if project != self.project_id:
                logger.warning(f"⚠️ Proyecto en credenciales ({project}) difiere del configurado ({self.project_id})")
            
            logger.info(f"✅ Vertex AI inicializado - Proyecto: {self.project_id}, Región: {self.location}")
            return True
