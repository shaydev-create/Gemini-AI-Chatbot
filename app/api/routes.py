...
def service_worker():
    ...
    # Obtener la ruta absoluta del directorio static del proyecto (no app/static)
...
def robots_txt():
    """Robots.txt para SEO."""
    return '''User-agent: *\nAllow: /\nSitemap: /sitemap.xml''', 200, {'Content-Type': 'text/plain; charset=utf-8'}
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    <url>
        <loc>{base_url}/chat</loc>
        <lastmod>{current_date}</lastmod>
        <changefreq>weekly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>{base_url}/manifest.json</loc>
        <lastmod>{current_date}</lastmod>
...
def favicon():
    ...
    try:
        ...
        response.headers['Cache-Control'] = 'public, max-age=86400'  # Cache por 1 día
        return response
    except FileNotFoundError:
...
def send_message():
    ...
    try:
        ...
        if len(user_message) > 4000:
            return jsonify({
                'success': False,
                'message': 'Mensaje demasiado largo (máximo 4000 caracteres)',
