...
def register_api_routes(app):
    ...
    def serve_e2e_test_files(filename):
        ...
        try:
            # Para archivos JavaScript, leer el contenido y devolverlo con el
            # tipo MIME correcto
            if filename.endswith(".js"):
                with open(
                    os.path.join(tests_e2e_dir, filename), "r", encoding="utf-8"
                ) as f:
                    content = f.read()
            ...
            current_app.logger.error(
                f"Error al servir archivo {filename}: {
                    str(e)}"
            )
                ...
                jsonify(
                    {
                        "success": False,
                        "message": (
                            "Mensaje demasiado largo " "(máximo 4000 caracteres)"
                        ),
