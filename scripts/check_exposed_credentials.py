...
def check_git_history():
    ...
    for commit in commits:
        ...
        commit_info = run_command(f"git show --name-only --format='%h|%an|%ad|%s' {commit} --date=short")
        if not commit_info:
            continue
        ...
        for cred_type, patterns in PATTERNS.items():
            for pattern in patterns:
                ...
                if matches:
                    ...
                    # Añadir al reporte
                    report_lines.append(f"⚠️  {cred_type} encontrada en commit {short_hash}")
                    report_lines.append(f"   Fecha: {date}, Autor: {author}")
                    report_lines.append(f"   Asunto: {subject}")
                    report_lines.append(f"   Credencial parcial: {', '.join(safe_matches)}")
                    report_lines.append("")
