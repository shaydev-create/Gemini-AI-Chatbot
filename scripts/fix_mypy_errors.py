...
class MyPyErrorFixer:
        ...
        if match:
            ...
            for imp in imports_needed:
                if imp not in existing_imports:
                    new_imports += f', {imp}'
            return re.sub(import_pattern, f'from typing import {new_imports}', content)
        return content
    ...
    def fix_optional_types(self, content: str) -> str:
        ...
        content = re.sub(
            r'(\w+):\s*int\s*=\s*None',
            r'\1: Optional[int] = None',
            content
        )

        content = re.sub(
            r'(\w+):\s*dict\s*=\s*None',
            r'\1: Optional[dict] = None',
            content
        )
    ...
    def fix_any_returns(self, content: str) -> str:
        ...
        content = re.sub(
            r'def\s+(\w+)\([^)]*\)\s*->\s*str:\s*\n([^}]*return\s+(?!")[^"\n]*)',
            lambda m: m.group(0).replace('-> str:', '-> str:') if 'str(' in m.group(2) else m.group(0).replace('-> str:', '-> Any:'),
            content,
            flags=re.MULTILINE | re.DOTALL
        )
