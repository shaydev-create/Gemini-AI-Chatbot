...
class TestVertexAIPostgreSQLIntegration:
    ...
    def test_daily_metrics_aggregation(self, setup_test_database):
        ...
        with engine.connect() as conn:
            ...
            logs = [
                {'cost': 0.001, 'input_tokens': 10, 'output_tokens': 15, 'source': 'vertex_ai', 'success': True},
                {'cost': 0.002, 'input_tokens': 20, 'output_tokens': 25, 'source': 'vertex_ai', 'success': True},
                {'cost': 0.001, 'input_tokens': 15, 'output_tokens': 20, 'source': 'gemini_api', 'success': True},
                {'cost': 0.000, 'input_tokens': 5, 'output_tokens': 0, 'source': 'vertex_ai', 'success': False}
            ]
    ...
    def test_vertex_ai_with_database_logging(self, mock_vertexai, setup_test_database, vertex_config):
    ...
    def test_full_integration_real_services(self):
        ...
        postgres_url = f"postgresql://{os.getenv('POSTGRES_TEST_USER')}:{os.getenv('POSTGRES_TEST_PASSWORD')}@{os.getenv('POSTGRES_TEST_HOST')}:{os.getenv('POSTGRES_TEST_PORT', '5432')}/{os.getenv('POSTGRES_TEST_DB')}"
        ...
        # Test respuesta real
        result = client.generate_response("Hola, este es un test de integración")
